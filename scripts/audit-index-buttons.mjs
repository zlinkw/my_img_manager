...
try {
  ...
  const mobileLibrary = await evaluate(client, `(() => {
    ...
    const mobileViewerBatchFocus = document.activeElement === document.getElementById('share-selected');
  ...
  assert.equal(mobileLibrary.mobileViewerBatchFocus, true, "mobile selection completion must focus the batch share action");
  ...
  await client.send("Page.navigate", { url: "data:text/html;charset=utf-8," + encodeURIComponent(`<!doctype html><html><head><meta charset="utf-8"><title>PDF 图片保存</title><style>html,body{margin:0;min-height:100%;background:#EEF2F7;color:#0F172A;font:14px system-ui,sans-serif}body{padding:28px;box-sizing:border-box}#pdf-image-saver-preferences{display:block;max-width:900px;margin:auto;padding:20px;border:1px solid #CBD5E1;border-radius:8px;background:#FFF}h1{margin:0 0 6px;font-size:20px}.audit-note{margin:0 0 18px;color:#64748B}</style></head><body><main id="pdf-image-saver-preferences"><h1>PDF 图片保存</h1><p class="audit-note">框选保存是主流程；自动识别可选；高级原图提取收在下方。每次打开都会重新读取当前设置并修复异常值。</p><section class="pdf-image-saver-prefs-section"><h2 class="pdf-image-saver-prefs-section-title">采集方式</h2><div class="pdf-image-saver-prefs-list"><div class="pdf-image-saver-prefs-row"><label for="pdf-image-saver-default-quality">默认预览质量</label><select id="pdf-image-saver-default-quality"><option value="low">低</option><option value="medium">中</option><option value="high">高</option></select><p>未知旧值恢复为推荐的中清晰度。</p></div><div class="pdf-image-saver-prefs-row"><label for="pdf-image-saver-default-category">默认图片类别</label><select id="pdf-image-saver-default-category"><option value="auto">自动判断</option><option value="heatmap">热图／矩阵图</option><option value="table">科研表格</option></select><p>未知旧值恢复为自动判断。</p></div><div class="pdf-image-saver-prefs-row"><label for="pdf-image-saver-duplicate-guard">避免重复保存</label><span><input id="pdf-image-saver-duplicate-guard" type="checkbox"> 同时检查本次会话和已保存内容</span><p>旧版 false 字符串保持关闭，而不是被误判为开启。</p></div></div></section><section class="pdf-image-saver-prefs-section"><h2 class="pdf-image-saver-prefs-section-title">自动识别上限</h2><div class="pdf-image-saver-prefs-list"><div class="pdf-image-saver-prefs-row"><label for="pdf-image-saver-min-auto-area">最小图片面积比例</label><input id="pdf-image-saver-min-auto-area" type="number" step="0.001" min="0.001" max="0.5"><p>留空恢复默认值 0.003。</p></div><div class="pdf-image-saver-prefs-row"><label for="pdf-image-saver-auto-max-images">每页最多候选图片</label><input id="pdf-image-saver-auto-max-images" type="number" min="1" max="50"><p>超过上限时按 50 张保存。</p></div></div></section><details class="pdf-image-saver-prefs-section pdf-image-saver-prefs-advanced" id="pdf-image-saver-prefs-helper-section"><summary class="pdf-image-saver-prefs-section-title"><span>高级原图提取（可选）</span><span class="pdf-image-saver-prefs-summary-note">常规采集无需配置</span></summary><div class="pdf-image-saver-prefs-advanced-body"><div class="pdf-image-saver-prefs-list"><div class="pdf-image-saver-prefs-row"><label for="pdf-image-saver-max-page-images">每页最多原图数</label><input id="pdf-image-saver-max-page-images" type="number" min="1" max="500"><p>非整数会显示并保存最接近的整数。</p></div></div></div></details></main></body></html>`) });
  await waitFor(async () => evaluate(client, "document.readyState === 'complete'"));
  ...
  await evaluate(client, `(() => { globalThis.Zotero={Prefs:{values:{'extensions.pdfImageSaver.defaultQuality':'ultra','extensions.pdfImageSaver.defaultImageCategory':'medical_scan','extensions.pdfImageSaver.duplicateGuard':'false','extensions.pdfImageSaver.minAutoImageArea':'','extensions.pdfImageSaver.autoDetectMaxImages':999,'extensions.pdfImageSaver.maxPageImages':12.7},writes:[],reads:[],globalWrites:[],get:function(key,global){this.reads.push({key:key,global:global});return this.values[key]},set:function(key,value,global){this.values[key]=value;this.writes.push({key:key,value:value});this.globalWrites.push(global)}}}; })()`);
  await evaluate(client, `(0,eval)(${JSON.stringify(preferencesSource)})`);
  const preferenceNormalization = await evaluate(client, `(() => {
    PdfImageSaverPreferences.init();const quality=document.getElementById('pdf-image-saver-default-quality');const category=document.getElementById('pdf-image-saver-default-category');const duplicate=document.getElementById('pdf-image-saver-duplicate-guard');const area=document.getElementById('pdf-image-saver-min-auto-area');const auto=document.getElementById('pdf-image-saver-auto-max-images');const page=document.getElementById('pdf-image-saver-max-page-images');return {quality:quality.value,category:category.value,duplicate:duplicate.checked,area:area.value,auto:auto.value,page:page.value,writes:Zotero.Prefs.writes,reads:Zotero.Prefs.reads,globalWrites:Zotero.Prefs.globalWrites,status:document.getElementById('pdf-image-saver-prefs-status').textContent,documentWidth:document.documentElement.scrollWidth,viewportWidth:innerWidth};
  })()`);
  ...
  assert.equal(preferenceNormalization.area, "0.003", "loading an empty numeric preference must visibly restore its default in a real DOM");
  assert.equal(preferenceNormalization.auto, "50", "loading an out-of-range numeric preference must visibly clamp to its maximum in a real DOM");
  assert.equal(preferenceNormalization.page, "13", "loading a fractional integer preference must visibly round in a real DOM");
  assert.deepEqual(preferenceNormalization.writes.map((write) => write.value), ["medium", "auto", false, 0.003, 50, 13], "loading stale preferences must persist only the normalized effective values shown to the user");
  assert.ok(preferenceNormalization.reads.length >= 6 && preferenceNormalization.reads.every((read) => read.global === true), "preference DOM must read the canonical global extension branch");
  assert.ok(preferenceNormalization.globalWrites.length === 6 && preferenceNormalization.globalWrites.every((global) => global === true), "preference DOM must write the canonical global extension branch");
  assert.ok(preferenceNormalization.status.includes("每页最多 50 张候选图"), "preference status must use the normalized automatic-candidate cap");
  ...
  const preferenceReopen = await evaluate(client, `(() => {
    Zotero.Prefs.writes.length=0;Object.assign(Zotero.Prefs.values,{'extensions.pdfImageSaver.defaultQuality':'high','extensions.pdfImageSaver.defaultImageCategory':'heatmap','extensions.pdfImageSaver.duplicateGuard':true,'extensions.pdfImageSaver.minAutoImageArea':0.008,'extensions.pdfImageSaver.autoDetectMaxImages':7,'extensions.pdfImageSaver.maxPageImages':22});PdfImageSaverPreferences.init();return {quality:document.getElementById('pdf-image-saver-default-quality').value,category:document.getElementById('pdf-image-saver-default-category').value,duplicate:document.getElementById('pdf-image-saver-duplicate-guard').checked,area:document.getElementById('pdf-image-saver-min-auto-area').value,auto:document.getElementById('pdf-image-saver-auto-max-images').value,page:document.getElementById('pdf-image-saver-max-page-images').value,writes:Zotero.Prefs.writes.length,status:document.getElementById('pdf-image-saver-prefs-status').textContent};
  })()`);
  const { status: preferenceReopenStatus, ...preferenceReopenValues } = preferenceReopen;
  ...
  assert.ok(preferenceReopenStatus.includes("默认质量：高") && preferenceReopenStatus.includes("每页最多 7 张候选图"), "reopened preference status must reflect externally changed values");
  if (screenshotDirectory) {
