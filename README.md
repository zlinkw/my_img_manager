...
- 查看器另有“矩形框选”和“手绘选区”。先放大图片，再沿所需图标或图形边缘拖动；松开后点“导出选区 SVG”。选区按原图像素坐标保存，缩放和平移后仍与图像对齐。手绘选区外透明；常见浅色纸面背景在占据选区边界多数时也会透明化。该导出只含选中的原图像素描摹结果，不含标注；局部描摹采用更精细的路径设置。若文字或照片仍落在选区内，它们也会被描摹；可缩小选区重新圈出所需形状。原图与整图导出按钮保留。
