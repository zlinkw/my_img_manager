...
1. 打开一篇 PDF。
2. 选区有可独立提取的矢量内容时保存原生 SVG；纯位图或混合内容保存原图的 PNG/JPEG 栅格版本。
3. 点击“框选保存”，或在 PDF 页面右键选择对应入口。
4. 拖出矩形范围。整页会变暗，绿色虚线显示实时范围，虚线内部保持正常亮度。
5. 在确认窗口中检查预览、类别、PPT 用途和自定义描述；清晰度选项只影响确认前的预览。
...
- 查看器仅保留笔刷、橡皮和文字；B/E/T 切换工具，点击已有文字可继续编辑。滚轮缩放，按住 Ctrl 拖动平移，标注与原图同步变换。工具栏提供“导出原图”和“导出近似矢量 SVG”：前者直接下载入库字节，后者对位图按需归并邻近色块、描摹多边形 SVG 轮廓并附上标注；原生 SVG 则保留原路径。描摹抑制孤立像素碎片以减少路径数量和打开耗时，不限制导出的 SVG 文件大小；实际耗时和可处理规模受设备内存与浏览器能力影响。描摹视窗使用原图真实像素尺寸，避免 PNG 的 DPI 元数据造成右侧和下方截断。描摹结果可无损缩放，但极小的孤立细节、像素里的文字和细线可能变形，不能恢复原 PDF 的字体或曲线。近似 SVG 不写回图库。
- 查看器可用“矩形框选”快速铺底，再用“选区画笔”多次补选、“选区橡皮”精细擦除；笔径可设为 1–64 原图像素。选区蒙版随图像同步缩放、平移，放大后便于修边。“导出选区原图 PNG”从图库原始字节裁出涂刷区域，保持原始像素颜色和擦出的透明孔洞，不执行描摹；它的分辨率受已保存原图限制。需要真正的路径时，另用“导出选区近似 SVG”以人工选区为裁剪轮廓描摹颜色区域；擦出的孔洞默认保持透明，只有勾选“填补内孔”才会补上封闭孔洞。PowerPoint 的“转换为形状”仅针对已有 SVG，不能把 PNG 自动变成高清矢量路径。
- 框选和整页保存确认前仍按选择的画质重新渲染预览；最终优先从原 PDF 提取纯矢量 SVG，位图选区保留 PNG/JPEG。单张矢量不超过 25 MB。`install:xpi` 会在关闭 Zotero 后把随包 Python、PyMuPDF、VTracer 和辅助脚本部署到插件配置目录。
- PDF 容器中的图片不一定是矢量图。纯嵌入位图直接转换为 SVG 只会嵌入像素；使用“导出近似矢量 SVG”才会生成独立的路径几何。
- 原 PDF 选区补取发生故障时，插件配置目录的 `last-region-capture.json` 记录最近一次执行阶段和错误，便于定位。
- 内置 Python 的文件路径按目录片段拼接，以兼容 Zotero 使用的 Gecko 路径 API。
...
插件已内置便携版 Python、PyMuPDF 和 VTracer，无需本机安装任何东西即可保存原生矢量图或导出近似矢量图。下面的命令只在你需要单独配置系统 Python 时才用得到：
