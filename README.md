...
- 查看器仅保留笔刷、橡皮和文字；B/E/T 切换工具，点击已有文字可继续编辑。滚轮缩放，按住 Ctrl 拖动平移，标注与原图同步变换。工具栏提供“导出原图”和“导出近似矢量 SVG”：前者直接下载入库字节，后者对位图按需精细描摹为 SVG 路径并附上标注；原生 SVG 则保留原路径。精细结果超过 25 MB 时自动回退到紧凑描摹。描摹视窗使用原图真实像素尺寸，避免 PNG 的 DPI 元数据造成右侧和下方截断。描摹结果可无损缩放，但像素里的文字和细线可能变形，不能恢复原 PDF 的字体或曲线。近似 SVG 不写回图库。
