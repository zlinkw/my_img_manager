...
- 查看器可用“矩形框选”快速铺底，再用“选区画笔”多次补选、“选区橡皮”精细擦除；笔径可设为 1–64 原图像素。选区蒙版随图像同步缩放、平移，放大后便于修边。“导出选区 SVG”直接沿人工选区轮廓生成单色路径，颜色取选中像素的主色；封闭的内部空洞默认填为该主色，外部保持透明。该方式优先保留形状，不重建照片、文字及形状内部的多色细节。原图与整图近似矢量导出仍可单独使用。
