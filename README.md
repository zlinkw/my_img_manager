...
1. 打开一篇 PDF。
2. 框选只保存纯矢量 SVG。选区只有位图或同时含有位图时，会说明原因并停止保存，不写入 PNG。
...
- 查看器仅保留笔刷、橡皮和文字；B/E/T 切换工具，点击已有文字可继续编辑。滚轮缩放，按住 Ctrl 拖动平移，标注与原图同步变换。查看器显示原图文件类型，导出状态和错误会直接显示在工具栏。导出标注 SVG 时，纯矢量原图的路径会内联到文件中，标注保留路径和文字；旧位图记录或含位图的 SVG 无法导出为纯矢量文件。
- 框选和整页保存确认前仍按选择的画质重新渲染预览；最终只从原 PDF 提取纯矢量 SVG，单张不超过 25 MB。`install:xpi` 会在关闭 Zotero 后把随包 Python、PyMuPDF 和辅助脚本部署到插件配置目录。
- PDF 容器中的图片不一定是矢量图。若选区在原 PDF 内只有嵌入位图，直接转换为 SVG 仍会嵌入原位图，因此插件会拒绝保存。插件不会把像素自动描摹成近似路径。
