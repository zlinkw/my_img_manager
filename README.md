...
1. 打开一篇 PDF。
2. 无需选择清晰度：选区含 PDF 绘图或文字时保存为 SVG；仅含嵌入位图时保存高分辨率 PNG，受 1.5 MB 单图上限约束。
...
- 框选保存优先按 600 DPI 重新渲染选区。若阅读器只能提供低分辨率画面，内置 PyMuPDF 会从原 PDF 补取更清晰的 PNG，并遵守 1.5 MB 单图上限。`install:xpi` 会在关闭 Zotero 后把随包运行时和辅助脚本部署到插件配置目录。
- 框选区含 PDF 绘图或文字时优先保存为真正的 SVG；纯位图 PDF 无法恢复为真正矢量图，SVG 标注文件中的原图也仍是位图。保存提示会显示实际格式和尺寸。插件自带 Python 和 PyMuPDF，无需本机安装。
