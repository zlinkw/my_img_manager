...
- 同一查看器另外提供矩形框选、手绘选区和导出选区 SVG。选区点在原图像素坐标中记录；缩放、平移时轮廓同步变换。桥接命令 `traceImage` 可携带经过校验的归一化 `trace_selection`，将选区 JSON 写入临时文件后交给内置描摹程序；程序只裁取并描摹选中像素，手绘边界外保持透明，边缘多数为浅色纸面时同色背景透明化。选区导出不合并标注，不写数据库，使用较细的描摹设置；未带选区时仍保留既有整图描摹行为。放在选区内的文字和照片会一起描摹。
