...
- 同一查看器提供矩形铺底、可多次叠加的选区画笔、精细选区橡皮和 1–64 原图像素笔径。选区蒙版在原图像素坐标中绘制，随缩放、平移同步变换。桥接命令 `traceImage` 可携带校验后的 PNG alpha 蒙版 `trace_selection`；内置辅助程序沿蒙版边界直接生成单色 SVG 路径，不再对选区执行 VTracer 颜色分层。封闭的未涂区域填为选中像素的主色，外部透明；导出不含文字标注、不写数据库、不保留照片和形状内部多色纹理。旧矩形与多边形 `trace_selection` 协议仍兼容；未带选区时仍使用既有整图描摹。
