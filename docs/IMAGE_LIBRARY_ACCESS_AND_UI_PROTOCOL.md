...
- 高清查看器由 OpenSeadragon 提供滚轮居中缩放、Ctrl 加拖动平移和左下角鸟瞰图。Fabric 仅提供笔刷、橡皮和文字标注，使用与图像相同的坐标变换；再次点击已有文字可直接编辑。快捷键 B/E/T，Esc 退出工具，Ctrl+Z 撤销。查看器提供导出原图与导出近似矢量 SVG 两个按钮。前者从 `readImageBytes` 下载入库字节；后者对位图通过 `traceImage` 先归并邻近色块再描摹多边形路径，减少孤立像素造成的碎路径，不设置 SVG 输出体积上限；原生 SVG 保留原路径，并合成标注。极小孤立细节及文字可能变化，不写入数据库。SVG 原图通过浏览器原生图像层显示。生成图库页时把 UMD 脚本内联进 HTML。缺失时回退为纯图片查看。
- 同一查看器提供矩形铺底、可多次叠加的选区画笔、精细选区橡皮和 1–64 原图像素笔径。选区蒙版在原图像素坐标中绘制，随缩放、平移同步变换。“导出选区原图 PNG”通过现有 `readImageBytes` 桥接取得图库原始字节，在浏览器内用同源 Blob 解码、按蒙版 alpha 裁剪并下载 PNG；保留原像素与透明孔洞，不描摹、不写数据库，不能凭空增加分辨率。`traceImage` 仍可携带校验后的 PNG alpha 蒙版 `trace_selection` 和布尔值 `fillHoles`；辅助程序沿蒙版边界生成裁剪轮廓，并对选中原图颜色描摹为 SVG 路径。透明内孔默认保留，只有显式勾选“填补内孔”时才以周围主色填补封闭内孔。旧矩形与多边形 `trace_selection` 协议仍兼容；未带选区时仍使用既有整图描摹。PowerPoint 的“转换为形状”只支持已有 SVG，不将 PNG 自动矢量化。
...
- `install:xpi` 在 Zotero 退出后将包内 Python、PyMuPDF 和辅助脚本部署到配置目录，并用运行时版本标记、导入探针和脚本哈希验证；运行时及脚本优先从该目录读取，不依赖系统 Python 或 XPI 内部资源读取成功。
