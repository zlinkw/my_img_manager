...
- 同一查看器提供矩形铺底、可多次叠加的选区画笔、精细选区橡皮和 1–64 原图像素笔径。选区蒙版在原图像素坐标中绘制，随缩放、平移同步变换。桥接命令 `traceImage` 可携带校验后的 PNG alpha 蒙版 `trace_selection` 和布尔值 `fillHoles`。内置辅助程序沿蒙版边界生成精确裁剪轮廓，并对选中原图的颜色区域描摹为路径；透明内孔默认保留，只有显式勾选“填补内孔”时才以周围主色填补封闭内孔。导出不含文字标注、不写数据库、不嵌入位图。旧矩形与多边形 `trace_selection` 协议仍兼容；未带选区时仍使用既有整图描摹。
