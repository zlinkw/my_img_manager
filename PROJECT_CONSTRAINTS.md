...
- New manual clips and whole-page captures save native SVG geometry without embedded bitmap images when available, up to 25 MB per image. Bitmap-only and mixed regions keep their original raster capture. Approximate SVG tracing is an explicit export operation and never replaces `images.image_blob`.
...
- Index-page behaviour is covered by `tests/current-release.test.js` together with the two browser audits (`audit:index-buttons` for the generated gallery, `audit:viewer` for the OpenSeadragon/Fabric engine). A standalone index/preview test file was retired rather than maintained: it drifted into asserting obsolete English markup and even APIs that no longer exist, and nothing ran it. Do not reintroduce a second, unexecuted test file; extend the coverage that `npm.cmd test` and the audits actually run.
